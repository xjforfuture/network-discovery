...
class natlas:
    ...
    def get_ip_route_table(self, dev_ip):
        node = natlas_node(dev_ip)
        if (node.try_snmp_creds(self.config.snmp_creds) == 0):
            return []
        routes = node.get_ip_route_table()
        if (routes == None):
            return []
    ...
    def topo_info(self):
        return [
            {
                'node_type': 'route' if node.router else 'switch',
                'node_name': node.name,
                'node_links': [
                    {
                        'remote_node_name': link.remote_name,
                ...
                'subnets': [
                    {
                        'ip': subnet.ip,
                        'mask': subnet.mask,
                        'interface_name': subnet.ifname,
                        'hosts': [
                            {
                                'host_name': host.host_name,
                                'host_ip': host.ip,
                                'host_mac': host.mac,
                                'host_os_info': host.os_info,
