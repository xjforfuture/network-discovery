...
def nic_info():
    gateway = netifaces.gateways()['default'][netifaces.AF_INET][0]  # 网关
    name = netifaces.gateways()['default'][netifaces.AF_INET][1]  # 网络适配器信息

    ip = None
    for interface in netifaces.interfaces():
        if interface == name:
            try:
                ip = netifaces.ifaddresses(interface)[netifaces.AF_INET][0]['addr']  # 获取IP
            except KeyError:
                pass

    with open('/etc/resolv.conf', 'r') as f:
        dns = seq(f.read().split('\n')).filter(lambda o: '#' not in o and 'nameserver' in o).list()[0].split(' ')[1]
...
class natlas_network:
    ...
    def discover(self, ip):
        ...
        node, new_node = self.query_node(ip, 'UNKNOWN')
        self.root_node = node
    ...
    def __discover_node(self, node, depth):
        ...
        route_neighbors = node.get_route_neighbors(network_obj=self)
        neighbors      = cdp_neighbors + lldp_neighbors + route_neighbors
        ...
        for n in neighbors:
            ...
            if (acl_action == 'include'):
                ...
                # discover this node
                child, query_result = self.query_node(n.remote_ip, n.remote_name)
