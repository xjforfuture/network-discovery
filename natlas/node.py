...
from .node_subnet import nodeSubNet
...
class natlas_ip_route:
    def __init__(self, dest, ifindex, ifname, next_hop, type, proto, mask):
        self.dest = dest
        ...
        self.mask = mask
    def __str__(self):
        return (f'<dest={self.dest},intf={self.intf},next_hop={self.next_hop},type={self.type}>,'
                f' proto={self.proto}, mask={self.mask}')
    def __repr__(self):
        return self.__str__()
...
class natlas_node:
        ...
        if self.ifip_vbtbl:
            for ifrow in self.ifip_vbtbl:
                for ifn, ifv in ifrow:
                    ifn = str(ifn)
                    if (ifn.startswith(OID_IF_IP_INDEX)):
        ...
        if self.ifip_vbtbl:
            for ifrow in self.ifip_vbtbl:
                for ifn, ifv in ifrow:
                    ifn = str(ifn)
                    if (ifn.startswith(OID_IF_IP_INDEX)):
                        if '.'.join(ifn.split('.')[10:]) == ip:
                            return ifv.prettyPrint()
    ...
    # Will always return an array.
    #
    def get_route_neighbors(self, network_obj):
        neighbors = []
        routes = self.get_ip_route_table()
        if not routes:
            return []
        ...
        for r in routes:
            if r.type == '4' and r.next_hop != '0.0.0.0':
                link = self.__get_node_link_info(r.ifindex)
                link.remote_ip = r.next_hop
                # todo name是设备的唯一标识，需要确定用什么作为设备的唯一标识
                # 这里先暂时用OID_SYSNAME作为name
                link.remote_name = self.snmpobj.get_val(OID_SYSNAME, link.remote_ip)

                print(f'link.remote_ip {link.remote_ip}, link.remote_name {link.remote_name}')

                child, query_result = network_obj.query_node(link.remote_ip, link.remote_name)

                link.discovered_proto = 'route'
                link.local_port = self.__get_ifname(r.ifindex)
                link.remote_port = child.__get_ifname(child.get_ifidx_from_ip(link.remote_ip))
                link.remote_plat = None
                link.remote_ios = None
                link.remote_mac = None

                neighbors.append(link)

            # 直连路由，子网
            if r.type == '3':
                arps = self.get_arp_table()

                def ip_to_int(ip):
                    return socket.ntohl(struct.unpack("I", socket.inet_aton(str(ip)))[0])

                mask = ip_to_int(r.mask)
                subnet = ip_to_int(r.dest) & mask
                hosts = [(arp.ip, arp.mac) for arp in arps if ip_to_int(arp.ip) & mask == subnet]
                self.subnets.append(nodeSubNet(self, r.dest, r.mask, r.ifindex, self.__get_ifname(r.ifindex), hosts))

        return neighbors


    def __get_node_link_info(self, ifidx, ifidx2=None):
        snmpobj = self.snmpobj
    ...
    def get_ip_route_table(self):
        # use cache if possible
        if (self.ip_route_vbtbl == None):
            self.ip_route_vbtbl = self.snmpobj.get_bulk(OID_IP_ROUTE)

        routes = []
        for r in self.ip_route_vbtbl:
            for n, v in r:
                tok = str(n).split('.')
                if (tok[9] == OID_IP_ROUTE_DEST.split('.')[9]):
                    dest = v.prettyPrint()
                    tmp = '.'.join(tok[10:])

                    ifindex = self.snmpobj.cache_lookup(self.ip_route_vbtbl, OID_IP_ROUTE_IFIDX+'.'+ tmp)
                    ifname = self.__get_ifname(ifindex)
                    next_hop = self.snmpobj.cache_lookup(self.ip_route_vbtbl, OID_IP_ROUTE_NEXT_HOP+'.'+ tmp)
                    type = self.snmpobj.cache_lookup(self.ip_route_vbtbl, OID_IP_ROUTE_TYPE + '.' + tmp)
                    proto = self.snmpobj.cache_lookup(self.ip_route_vbtbl, OID_IP_ROUTE_PROTO + '.' + tmp)
                    mask = self.snmpobj.cache_lookup(self.ip_route_vbtbl, OID_IP_ROUTE_MASK + '.' + tmp)

                    ip_route_obj = natlas_ip_route(dest, ifindex, ifname, next_hop, type, proto, mask)
                    routes.append(ip_route_obj)
        return routes if routes else []


    def get_sys_obj_id(self):
        # use cache if possible
        if (self.sys_obj_id == None):
            self.sys_obj_id = self.snmpobj.get_bulk(OID_SYS_OBJ_ID)

        for r in self.sys_obj_id:
            for n, v in r:
                return str(v)
