...
OID_IF_IP_ADDR          = '1.3.6.1.2.1.4.20.1.1'
OID_IF_IP_INDEX         = '1.3.6.1.2.1.4.20.1.2'                    # + a.b.c.d = ifid
...
class natlas_snmp:
    ...
    #
    def get_val(self, oid, ip=None):
        cmdGen = cmdgen.CommandGenerator()
        errIndication, errStatus, errIndex, varBinds = cmdGen.getCmd(
                        cmdgen.CommunityData(self.v2_community),
                        cmdgen.UdpTransportTarget((ip if ip else self._ip, SNMP_PORT), retries=2),
    ...
    #
    def get_bulk(self, oid, ip=None):
        cmdGen = cmdgen.CommandGenerator()

        try:
            errIndication, errStatus, errIndex, varBindTable = cmdGen.bulkCmd(
                            cmdgen.CommunityData(self.v2_community),
                            cmdgen.UdpTransportTarget((ip if ip else self._ip, SNMP_PORT), timeout=3, retries=2),
                            0, 50,
                            oid,
                            lookupNames = False, lookupValues = False
            )
        ...
        if errIndication:
            print('[E] get_snmp_bulk(%s): %s %s' % (self.v2_community, errIndication, oid))
        else:
            ret = []
